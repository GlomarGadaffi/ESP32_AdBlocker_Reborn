...
### M7 — DoT server/SNI change never reconnects the persistent TLS session ✅ (#95)
`dot.c:181-196` — `dot_set()` wrote `s_enabled`/`s_server`/`s_sni`, persisted them to NVS, and called `worker_start_once()`, which opens with `if (s_worker_up) return;` — so once the DoT worker task exists, `dot_set()` had no effect on it. `conn_ensure()` (48-50) opens `if (s_conn) return true;` and only reads `s_server`/`s_sni` when building a brand-new session, and `conn_drop()`'s only caller was a failed round-trip in `dot_task`. Result: changing the DoT server/SNI in the UI updated NVS and the displayed value, but every live query kept riding the OLD TLS session to the OLD resolver indefinitely — steady LAN traffic keeps a TLS session from idle-closing, so this had no natural recovery. Disabling DoT didn't help either: the session just stayed parked, and re-enabling hit the same `s_worker_up` early-return. Minor accompanying bug: `s_server`/`s_sni` were plain `static char[64]`, written by the httpd task and read by the worker task with no synchronization — a config write landing mid-`conn_ensure()` could hand one handshake attempt a torn host/SNI (bounded, NUL-terminated, not memory-unsafe, but still wrong).
**Fix:** a `s_reconnect` atomic bool (`_Atomic bool`, same idiom as `blocklist.c`'s `s_paused`) is set by `dot_set()` only when enabled/server/SNI actually *change* value — a Save with unchanged fields is a no-op, not a forced reconnect. `dot_task` checks the flag once per loop iteration, before it can touch `conn_ensure()`, and the check sits *between* requests: a query already in flight finishes (or fails and falls back to UDP) on the old session before the drop happens, so no round-trip is aborted mid-flight. `s_server`/`s_sni` are now guarded by a `SemaphoreHandle_t s_cfg_mutex` (the `acl.c`/`rewrite.c` mutex-protected-config pattern): `dot_set()` writes under the lock and computes `changed` there too; `conn_ensure()` snapshots both fields into local stack buffers under the same lock before using them to connect, closing the torn-read window. NVS writes stay outside the lock, matching the rest of the codebase (single httpd task is the only writer, so there's nothing to race). The Upstream tab's "Save & apply (restart DNS task)" button label was checked against this fix and left as-is — it doesn't literally restart `dns_task`, but a save now does apply to the live DoT session, which is what the label promises to a user.
**Verified on hardware 2026-09-05 (Waveshare, PR #119 image).** Switched the live DoT resolver from Cloudflare to Quad9 through the web UI and the serial log showed `persistent DoT session up (9.9.9.9, SNI dns.quad9.net)`, with resolution unbroken across the change. Switching back produced `persistent DoT session up (1.1.1.1, SNI one.one.one.one)` — but only after forcing *uncached* queries: the first attempt looked like a failed reconnect purely because the probe names were already in the forward cache, so the worker had no reason to send anything upstream. Worth remembering when testing this again: a cache hit proves nothing about which resolver is live.
...
**Verified on hardware 2026-09-05 (Waveshare, PR #119 image).** The earlier round of #48 testing passed 19/19 and still missed this, because every domain it used was *directly* on a feed — the test set, not the method, was the weak point. Building a real cloaking case needs care: custom rules match subdomain-inclusively, so blocking `github.com` also blocks `www.github.com` as a **direct** hit and proves nothing about the chain. The case used instead was `www.microsoft.com`, which CNAMEs to `e13678.dscb.akamaiedge.net`, with a custom rule of `akamaiedge.net` — a different registrable domain, so the only route to a block is the chain. Results: with the rule armed and no pause, blocked (`0.0.0.0`); paused for this client, the real address over **both UDP and TCP**; after resume, blocked again, so nothing was cached. Two-client isolation was checked with a genuine second host rather than an assumption: with the pause applied only to 192.168.12.110, that host got `23.220.200.101` while 192.168.12.161 kept getting `0.0.0.0` for the same name at the same time. The #48 suites were re-run against this image unchanged (12/12 web-form, 7/7 scope/TCP/expiry).
