...
static esp_err_t handle_metrics_view(httpd_req_t *r)
{
    ...
    " if(!LAST){a.textContent='waiting for the first sample';return;}\n"
    " var s=Math.round((Date.now()-LAST)/1000);\n"
    " a.textContent='updated '+(s<2?'just now':s+'s ago');\n"
    ...
    "  return r.json();\n"
    " }).then(function(j){if(j){render(j);LAST=Date.now();}mark();})\n"
    " .catch(function(e){console.error('metrics view:',e);mark();});}\n"
